...
export const RENPY = (() => {
  ...
  function applyTranslations(source, dialogs, eol, creditLine) {
    ...
    for (const d of dialogs) {
      const chosen = d.manualMasked ?? d.mtMasked;
      if (chosen == null) continue;
      const unmasked = unmaskTagsInText(chosen, d.placeholderMap);
      reps.push({
        start: d.contentStart,
        end: d.contentEnd,
        value: escapeForRenpyString(unmasked, d.quoteChar, d.isTriple),
      });
    }
    ...
    const hasCredit = credit && out.includes(credit);
    if (credit && !hasCredit) out = out + nl + nl + credit + nl;
    else out = out + nl;
    return out;
  }
