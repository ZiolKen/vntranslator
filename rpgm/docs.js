(()=>{ 
  ...
  const norm=(href)=>{
    try{
      ...
      return u.pathname.replace(/\/+/g,'/').split('/').filter(Boolean).pop()||'';
    }catch(_){
      return String(href||'').split('#')[0].split('?')[0].split('/').filter(Boolean).pop()||'';
    }
  };
  ...
  const buildSidebar=()=>{
    ...
    groups.forEach((g)=>{
      ...
      (g.items||[]).forEach((it)=>{
        ...
        flat.push({id,title:(it.title||it.label||it.href||'Untitled'),href:a.getAttribute('href')});

        if(id && id===currentFile){
  ...
  const bindSearch=()=>{
    ...
    const render=(q)=>{
      ...
      list.forEach((it,i)=>{
        ...
        m.textContent=cfg.categoryLabel||cfg.category||'';
        a.appendChild(t);
        a.appendChild(m);
        a.addEventListener('click',()=>{
          document.body.classList.remove('is-searchOpen');
