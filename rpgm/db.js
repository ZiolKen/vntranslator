(function () {
  ...
  const DB_NAME = 'vntranslator-rpgm';
  const DB_VERSION = 1;
  ...
  async function createSession(input) {
    ...
    const fileName = String(input?.fileName || 'file.json');
    const originalJsonText = String(input?.originalJsonText || '');
    ...
    for (let i = 0; i < dialogs.length; i++) {
      const d = dialogs[i] || {};
      const original = typeof d.original === 'string' ? d.original : '';
      const translated = typeof d.translated === 'string' ? d.translated : '';
      dialogStore.put({
        sessionId: id,
        idx: i,
        code: typeof d.code === 'string' || typeof d.code === 'number' ? d.code : '',
        original,
