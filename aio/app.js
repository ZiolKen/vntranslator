...
import { ENGINES, ENGINE_ORDER, getEngine, detectEngine, extractDialogs as extractDialogsFor, applyTranslations as applyTranslationsFor, hasPlaceholderWarning } from './registry.js';

const DEFAULT_ENGINE = 'renpy';

function fileEngine(f) {
  return (f && f.engine && ENGINES[f.engine]) ? f.engine : DEFAULT_ENGINE;
}
...
function isSupportedFile(name) {
  ...
  return ['.rpy', '.json', '.ks', '.ast', '.asb', '.txt'].includes(ext);
}
...
const ui = {
  ...
  scriptEngineSelect: el('scriptEngineSelect'),
  extractModeRow: el('extractModeRow'),
...
function updateRowDOM(rowIndex) {
  ...
  if (status) {
    ...
    const warn = warnOn && hasPlaceholderWarning(fileEngine(f), v);
...
function getMetaKind(d, engineId) {
  const tr = String(d.translated ?? '');
  if (!tr.trim()) return 'empty';
  if (hasPlaceholderWarning(engineId || DEFAULT_ENGINE, tr)) return 'error';
  return 'ok';
}
...
function computeActiveView() {
  ...
  for (let i = 0; i < f.dialogs.length; i++) {
    ...
    if (filterMode === 'error' && getMetaKind(d, engineId) !== 'error') continue;
...
function renderRow(f, idx, warnOn) {
  ...
  const warn = warnOn && hasPlaceholderWarning(engineId, String(trText));
  ...
  function refreshMeta() {
    ...
    const err = warnOn2 && hasPlaceholderWarning(engineId, v);
  ...
  
  {
    ...
    const warn2 = warnOn && hasPlaceholderWarning(engineId, v);
  ...
  ta.addEventListener('input', () => {
    ...
    const warnNow = !!ui.showWarnings.checked && hasPlaceholderWarning(engineId, v);
...
const scheduleSaveActiveFile = debounce(async () => {
  ...
  const payload = {
    ...
    eol: f.eol,
    engine: fileEngine(f),
    dialogs: f.dialogs.map(d => ({ ...d, translated: d.translated ?? null, flagged: !!d.flagged })),
  };
  ...
  try {
    ...
    for (const f of files) {
      ...
      state.files.set(f.path, {
        ...
        eol: f.eol || '\n',
        engine: (f.engine && ENGINES[f.engine]) ? f.engine : detectEngine(f.path, String(f.source ?? '')),
...
function applyExtractMode(engineId) {
  const eng = getEngine(engineId || DEFAULT_ENGINE);
  if (eng && eng.supportsExtractMode && typeof eng.core.setMode === 'function') {
    eng.core.setMode(ui.extractMode.value);
  }
}

function resolveImportEngine(fileName, text) {
  const picked = ui.scriptEngineSelect ? ui.scriptEngineSelect.value : 'auto';
  if (picked && picked !== 'auto') return picked;
  return detectEngine(fileName, text);
}
...
async function importFiles(fileList) {
  const items = Array.from(fileList || []).filter(f => f && isSupportedFile(f.name || ''));
  ...
  for (const file of items) {
    ...
    const engineId = resolveImportEngine(path, normalized);

    applyExtractMode(engineId);
    const dialogs = extractDialogsFor(engineId, normalized, path);

    const fileState = { path, source: normalized, eol, engine: engineId, dialogs };
    ...
    if (ui.autoSave.checked) {
      await Store.saveFile(PROJECT_ID, path, {
        ...
        eol,
        engine: engineId,
        dialogs: dialogs.map(d => ({ ...d, translated: d.translated ?? null, flagged: !!d.flagged })),
      });
    }
...
ui.extractMode.addEventListener('change', () => {
  ...
  if (p) {
    ...
    if (f) {
      ...
      if (!eng.supportsExtractMode) {
        log(`${eng.label} does not support extract-mode tiers; ignoring.`, 'warn');
        return;
      }
      applyExtractMode(engineId);
      f.dialogs = extractDialogsFor(engineId, f.source, f.path);
      syncFileMeta(f);
      if (ui.autoSave.checked) scheduleSaveActiveFile();
      renderTable();
      scheduleRefreshSidebar();
      log('Re-extracted current file using mode: ' + eng.core.getMode());
    }
  }
});

if (ui.scriptEngineSelect) {
  ENGINE_ORDER.forEach((id) => {
    const opt = document.createElement('option');
    opt.value = id;
    opt.textContent = ENGINES[id].label;
    ui.scriptEngineSelect.appendChild(opt);
  });
  const autoOpt = document.createElement('option');
  autoOpt.value = 'auto';
  autoOpt.textContent = 'Auto-detect (by extension/content)';
  ui.scriptEngineSelect.insertBefore(autoOpt, ui.scriptEngineSelect.firstChild);
  ui.scriptEngineSelect.value = 'auto';

  ui.scriptEngineSelect.addEventListener('change', () => {
    const manual = ui.scriptEngineSelect.value !== 'auto';
    if (ui.extractModeRow) {
      const eng = manual ? getEngine(ui.scriptEngineSelect.value) : null;
      ui.extractModeRow.style.display = (!manual || eng?.supportsExtractMode) ? '' : 'none';
    }
  });
}
...
async function translateDialogs(path, indices) {
  const f = state.files.get(path);
  if (!f) return;
  const scriptEngine = getEngine(fileEngine(f));
  ...
  for (let start = 0; start < list.length; start += batch) {
    ...
    for (let i = 0; i < slice.length; i++) {
      ...
      const unmasked = scriptEngine.unmaskTagsInText(out, d.placeholderMap);
...
ui.btnExportFile.addEventListener('click', () => {
  ...
  const engineId = fileEngine(f);
  const out = applyTranslationsFor(engineId, f.source, f.dialogs, '\n');
  const restored = restoreLineEndings(out, f.eol);
  const base = p.split('/').pop();
  makeDownload(base || 'translated.txt', restored);
});
