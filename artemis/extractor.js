(function () {
  ...
  document.getElementById('extractBtn').addEventListener('click', async () => {
    ...
    if (!files.length) return alert('Select at least one Artemis file.');
    for (const file of files) {
      ...
      downloadText(file.name.replace(/\.[^.]+$/, '') + '.artemis-dialogs.json', JSON.stringify(pack, null, 2));
      log('Extracted ' + extracted.lines.length + ' strings from ' + file.name, 'success');
    }
  });
  ...
  document.getElementById('mergeBtn').addEventListener('click', async () => {
    ...
    if (!translated || !original) return alert('Select translated JSON + original AST script.');
    const pack = JSON.parse(await translated.text());
    const { text, detected } = await readFileAsText(original);
    const merged = Parser.insertArtemisTextBack(text, pack.lines || [], pack.mapping || []);
