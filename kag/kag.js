(function () {
  ...
  if (!Parser || !Common) {
    throw new Error('Missing VNKAGParserCore or VNTranslationCommon');
  }
  ...
  function buildMessages(lines) {
    ...
    return [
      { role: 'system', content: ['You are a veteran KiriKiri and KAG visual novel translator and localization specialist.',
      'Preserve all KAG tags, placeholders, escape sequences, variables, ruby markup, and speaker formatting exactly.',
      'Never translate commands, labels, control tags, inline bracket tags, @ commands, or script syntax.',
  ...
  async function start() {
    ...
    if (!files.length) {
      log('Select at least one .ks/.txt file first.', 'error');
      return;
    }
    ...
    try {
      ...
      for (const file of files) {
        ...
        log('Parsed ' + file.name + ' → ' + extracted.lines.length + ' dialog entries.', 'success');
      }
      ...
      parsedFiles.forEach((file) => {
        ...
        const merged = Parser.insertKAGTextBack(file.source, translatedLines, file.mapping);
      ...
      downloadBlob(blob, 'vntranslator-kag-' + Date.now() + '.zip');
      log('Done. Exported translated KAG ZIP.', 'success');
